...
// annotationGKENodeRecyclerState is the key of the annotation to use to store the expiry datetime
const annotationGKENodeRecyclerState string = "estafette.io/gke-node-recycler-state"

// GKENodeRecyclerState represents the state of gke-node-recycler
type GKENodeRecyclerState struct {
...
var (
	...
	nodeTotals = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "estafette_gke_node_recycler_node_totals",
...
func main() {
	...
	log.Logger = zerolog.New(os.Stdout).With().
		Timestamp().
		Str("app", "estafette-gke-node-recycler").
		Str("version", version).
		Logger()
	...
	log.Info().
		...
		Msg("Starting estafette-gke-node-recycler...")
	...
	go func(waitGroup *sync.WaitGroup) {
		for {
			log.Info().Msg("Listing all nodes for cluster...")
			...
			if err != nil {
				log.Error().Err(err).Msg("Error while getting the list of nodes")
				log.Info().Msgf("Sleeping for %v seconds...", sleepTime)
				time.Sleep(time.Duration(sleepTime) * time.Second)
				continue
			}

			log.Info().Msgf("Cluster has %v nodes", len(nodes.Items))
...
func getCurrentNodeState(node *apiv1.Node) (state GKENodeRecyclerState) {
	var ok bool

	state.ExpiryDatetime, ok = node.Metadata.Annotations[annotationGKENodeRecyclerState]
...
func getDesiredNodeState(k KubernetesClient, node *apiv1.Node) (state GKENodeRecyclerState, err error) {
	...
	log.Info().
		Str("host", *node.Metadata.Name).
		Msgf("Annotation not found, adding %s to %s", annotationGKENodeRecyclerState, state.ExpiryDatetime)

	err = k.SetNodeAnnotation(*node.Metadata.Name, annotationGKENodeRecyclerState, state.ExpiryDatetime)
